...
`the little dachshund.app` 是主要管理入口，打开后显示为“小腊肠”。日常状态、账号、额度和代理确认都在 App 里查看。
...
- 配置 Codex 使用本地代理
- 如果检测到旧后台、旧运行目录或版本不一致，会自动迁移并同步当前 App 内置版本
...
如果代理离线或拖入新版 App 后后台版本没有更新，点击“启动/修复/更新后台”。
...
|------|------|
| 刷新 | 重新读取代理、账号和配置状态。 |
| 启动/修复/更新后台 | 安装、修复或更新后台代理服务。首次安装、代理离线、旧版迁移或版本不一致时先点它。 |
...
| 应用更新 | 高级操作：显式同步 App 内置运行资源，并重启代理。正在进行的 Codex 请求可能中断；如果新版本没有正常启动，App 会尝试恢复更新前的运行资源。 |
