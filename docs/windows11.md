...
.\platforms\windows\build_windows.ps1 -Version 0.6.6
```
...
- `dist\CodexProxyControlSetup-0.6.6-win-x64.exe`（已安装 Inno Setup 时）

如果只想生成便携文件：

```powershell
.\platforms\windows\build_windows.ps1 -Version 0.6.6 -SkipInstaller
```
...
```text
https://github.com/Fank1ng/codexproxyapi/releases/tag/v0.6.6
```
...
```powershell
Unblock-File .\dist\CodexProxyControlSetup-0.6.6-win-x64.exe
```
