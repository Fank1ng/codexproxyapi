...
`小腊肠.app` 是 macOS 主要管理入口。浏览器里的 Web 页面只用来查看状态。

macOS 使用 `小腊肠.app`，Windows 11 使用
`CodexProxyControlSetup-0.5.1-win-x64.exe` 安装控制端。
...
- 准备运行目录：`~/Library/Application Support/xiaolachang`
- 安装用户级后台服务：`~/Library/LaunchAgents/com.fank1ng.xiaolachang.plist`
- 配置 Codex 使用本地代理
...
```text
CodexProxyControlSetup-0.5.1-win-x64.exe
```
...
```powershell
Unblock-File .\CodexProxyControlSetup-0.5.1-win-x64.exe
```
...
```text
~/Library/LaunchAgents/com.fank1ng.xiaolachang.plist
```
...
```bash
open "$HOME/Library/Application Support/xiaolachang/proxy.log"
```

查看运行目录：

```bash
open "$HOME/Library/Application Support/xiaolachang"
```
...
|------|------|
| 刷新 | 重新读取代理、账号和配置状态。 |
| 启动 | 安装或修复后台代理服务，并尝试启动代理。代理离线时先点它。 |
...
| 应用更新 | 同步 App 内置运行资源，并重启代理。正在进行的 Codex 请求可能中断；如果新版本没有正常启动，App 会尝试恢复更新前的运行资源。 |
...
```text
~/Library/Application Support/xiaolachang/accounts/
```
...
如果“应用更新”失败，结果里可能会显示备份路径。请保留该路径并打开日志，方便维护者确认是否已经回滚成功。
